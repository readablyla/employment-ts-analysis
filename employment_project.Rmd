...
```
So we only need rows 180-503 for the total data set, for the predictions we need 493 to 503. we should use this plot in the intro section for our intitial time series. 
```{r}
trunc_dat <- dat[180:492,]
trunc_dat1 <- dat[180:503,]
trunc_dat2 <- dat[491:503,]
...
exp_ts <- ts(trunc_dat2[, 2], start = c(2018, 12), end = c(2019, 12), frequency = 12)#for mape calcs
plot.ts(ts_dat, ylab="People Employed (000's)") #updated units
plot.ts(ts_tot, ylab="People Employed (000's)") #updated units
plot.ts(exp_ts)
...
```
First model has the lowest AICc but the Ljung-Box statistic is not satisfactory at lag 20,  and unnesssecary coefficients were present in the model.
...
```{r}
ts.plot(cbind(ts_dat,ts_tot, exp(fore$pred)),lwd = 1, col = c(1,1,2), xlim = c(1993, 2021), ylab = "original", ylim = c(7500, 13000) )
#lines(exp(fore$pred), type = "p", col = 2)
lines(exp(fore$pred+2*fore$se), lty="dashed", col=4) #95% confidence boundaries
lines(exp(fore$pred-2*fore$se), lty="dashed", col=4) #95% confidence boundaries
```
...
```{r}
ts.plot(cbind(ts_dat,exp(fore$pred)), lwd = c(2,2), col = c(1,2), xlim = c(2018, 2020), ylab = "People Employed (000's)", ylim = c(12300, 13500) )
lines(exp(fore$pred), type = "p", col = 2, lwd = 2)
#95% confidence boundaries 
lines(exp(fore$pred+2*fore$se), lty="dashed", col=4, lwd = 2)
lines(exp(fore$pred-2*fore$se), lty="dashed", col=4, lwd = 2)
lines(exp_ts, lty = "dashed", lwd = 2) 
```
Concerned about the dashed line starting in december 2018 and our predictions starting in january...

Assess the predictions. MAPE etc exp_ts is defined with the other data set breaks
```{r}
MAPE = 0
for (i in 1:12){
  MAPE = MAPE + abs((exp_ts[i] - exp(fore$pred[i]))/exp_ts[i]) 
}
MAPE = MAPE/12*100
MAPE
```
Mape seems pretty good, yes?
